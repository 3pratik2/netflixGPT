    ...
    <div className="relative group w-[140px] md:w-[170px] lg:w-[190px] flex-shrink-0 cursor-pointer">
      ...
      <div
        className={`
          ...
          flex flex-col justify-end p-3 text-white z-50 pointer-events-none
        `}
        style={{ transformOrigin: index < 2 ? "left" : "center" }} // shift origin for edge cards
      >
        {/* Title */}
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button className="bg-white text-black px-2 py-1 text-xs md:text-sm rounded-md flex items-center gap-1 hover:bg-white/80">
            <FaPlay className="w-3 h-3 md:w-4 md:h-4" /> Play
          </button>
          <button
            onClick={() => setExpanded(!expanded)}
            className="bg-gray-700/70 px-2 py-1 text-xs md:text-sm rounded-md flex items-center gap-1 hover:bg-gray-600/80"
          >
